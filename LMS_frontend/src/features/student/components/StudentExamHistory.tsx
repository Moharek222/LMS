...
export const StudentExamHistory: React.FC = () => {
  ...
  const { data, isLoading, isError, error, refetch } = useStudentExamHistory({
    page,
    limit,
  });
  ...
  if (isLoading) {
    return (
      <div className="bg-white rounded-3xl p-12 border border-slate-200 shadow-xs flex flex-col items-center justify-center text-center space-y-3 min-h-65">
        ...
        <p className="text-xs font-bold text-slate-600">جاري تحميل سجل الامتحانات...</p>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="rounded-3xl p-8 border border-red-200 bg-red-50/40 shadow-xs flex flex-col items-center justify-center text-center space-y-3">
        ...
        <h4 className="text-base font-bold text-slate-800">حدث خطأ أثناء تحميل سجل الامتحانات</h4>
        <p className="text-xs text-slate-600 font-semibold max-w-md">
          {toArabicErrorMessage(error, 'تعذر تحميل سجل الامتحانات حالياً.')}
        </p>
        <button
          onClick={() => refetch()}
  ...
  if (historyItems.length === 0) {
    return (
      <div className="bg-white rounded-3xl p-10 border border-slate-200 shadow-xs text-center space-y-3">
        ...
        <h4 className="text-base font-bold text-slate-800">سجل الامتحانات الشاملة</h4>
        <p className="text-xs text-slate-500 font-semibold max-w-sm mx-auto">
          لم تقم بتقديم أي امتحانات حتى الآن
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-xs flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-extrabold text-slate-800">سجل الامتحانات الشاملة</h3>
            <p className="text-xs text-slate-500 font-medium">عرض نتائج وتقييمات الامتحانات الشاملة السابقة</p>
          </div>
        </div>
