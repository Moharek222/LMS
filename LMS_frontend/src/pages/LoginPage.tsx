...
export const LoginPage: React.FC<LoginPageProps> = ({ onNavigateToRegister }) => {
  ...
  const { user, isAuthenticated, loginTeacher, loginStudent, logout, isLoading } = useAuth();
      ...
      <div className="lg:col-span-2 flex flex-col justify-center items-center px-6 py-8 sm:px-10 lg:px-12 bg-[#FAFBFC] overflow-y-auto h-full z-10 shadow-lg">
        <div className="w-full max-w-md my-auto">
          ...
          {isAuthenticated && user ? (
            <div className="flex flex-col items-center text-center p-6 bg-white rounded-2xl border border-slate-200 shadow-sm space-y-5">
              <div className="w-16 h-16 rounded-full bg-teal-50 flex items-center justify-center text-[#0D8A82] border-2 border-[#0D8A82]">
                <UserCheck size={32} />
              </div>

              <div>
                <h3 className="text-xl font-extrabold text-slate-800">أهلاً بك، {user.name} 👋</h3>
                <p className="text-sm text-slate-600 mt-1">
                  أنت مسجل الدخول حالياً بصفتك:{' '}
                  <strong className="text-[#0D8A82]">
                    {user.role === 'student' ? 'طالب' : user.role === 'teacher' ? 'مدرس' : 'مدير نظام'}
                  </strong>
                </p>
              </div>

              <div className="flex items-center gap-2 px-4 py-2 bg-slate-100 rounded-xl text-xs text-slate-700 font-semibold w-full justify-center">
                <CheckCircle2 size={18} className="text-[#0D8A82]" />
                <span>جلسة العمل نشطة ومفعلة</span>
              </div>

              <button
                onClick={logout}
                disabled={isLoading}
                className="w-full bg-slate-800 hover:bg-slate-900 text-white font-bold py-3 rounded-xl transition flex items-center justify-center gap-2 cursor-pointer"
              >
                <LogOut size={18} />
